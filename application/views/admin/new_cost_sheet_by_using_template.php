<div class="container-fluid page-body-wrapper">
      ...
      <div class="content-wrapper">
          ...
          <h3 class="page-title">
            Create Cost Sheet By Using Template (<?= $insert_id; ?>)
          </h3>
      	</div>
                  ...
                  <div class="row card-body">
                      <div class="col-3">
                        <form method="post" id="customerForm">
                          ...
                          <input type="hidden" name="CostSheetId" value="<?= $insert_id; ?>">
                        </form>
                      </div>
                      <div class="col-3">
                          ...
                          <form method="post" id="paymentTerms">
                            ...
                            <select class="form-control select2" onchange="updateData('paymentTerms','UpdatePaymentTerms');" name="payment_terms"  id="payment_terms">
                              ...
                              <?php if(!empty($payment_termss)) { ?>
                                <option value="<?php echo $payment_termss->payment_terms; ?>"><?php echo $payment_termss->pterms1; ?></option>
                                <option value="<?php echo $payment_termss->payment_terms2; ?>"><?php echo $payment_termss->pterms2; ?></option>
                                <option value="<?php echo $payment_termss->payment_terms3; ?>"><?php echo $payment_termss->pterms3; ?></option>
                              <?php } ?>                              
                            </select>
                            <input class="form-control select2" type="hidden" name="CostSheetId" value="<?= $insert_id; ?>">
                          </form>
                      </div>
                      <div class="col-3">
                        <form method="post" id="sales_person">
                          ...
                          <select class="form-control select2" onchange="updateData('sales_person','UpdateSalesPerson');" name="salesPerson"  id="salesPerson">
                            ...
                            <?php foreach ($salesperson as $key => $sales) { ?>
                              <option value="<?php echo $sales['id'] ?>"><?php echo $sales['name']; ?></option>
                            <?php } ?>
                          </select>
                         <input type="hidden" name="CostSheetId" value="<?= $insert_id; ?>">
                        </form>
                  ...
                  <div class="col-3">
                      <form method="post" id="currencyForm">
                        <label for="inputPassword4" class="">Currency</label>
                          <select class="form-control select2" onchange="updateDataCurrency('currencyForm','UpdateCurrency',this.value, <?= $convertCost->convert_value; ?>);" name="currency"  id="currency">
                                <option value="">Select</option>
          ...
          <div>
            <div class="row">
              <div class="col-lg-12 admin-accord">
                  ...
                  <?php $i =1; foreach ($cost_sheet_cat as $key => $value) { 
                    ...
                    <div class="card">
                      ...
                      <div id="collapse-<?= $i; ?>" class="collapse card-body" role="tabpanel" aria-labelledby="heading-<?= $i; ?>" data-parent="#accordion-2" style="">
                        <?php 
                          $query = "select cE.id, cE.title, cE.quantity, cE.unit, (SELECT SUM(total_cost) FROM cost_sheet_line_item WHERE sub_cat_id = cE.id) as sumTotalCost, (SELECT SUM(selling_price) FROM cost_sheet_line_item WHERE sub_cat_id = cE.id) as sumSellingCost from costsheet_subcategory cE where cE.costsheet_id = '".$this->uri->segment(5)."' AND cE.cat_id = '".$value['id']."'";
                          $subCategory= $this->db->query($query)->result_array();
                          $j= 1; 
                          foreach ($subCategory as $key => $subvalue) {  
                            $lineItems = $this->site_model->get_rows_c3('cost_sheet_line_item','cost_sheet_id',$this->uri->segment(5),'sub_cat_id',$subvalue['id'],'status',1);
                            ?>
                            <div id="accordion-1">
                              <div class="card">
                                    ...
                                    <a class="collapsed" role="button" data-toggle="collapse" href="#collapse-<?=$i?>-<?=$j?>" aria-expanded="false" aria-controls="collapse-1-1">
                                      ...
                                      <div class="aa-bb" style="top: 0px;">
                                        <?php if(!empty($subvalue['sumTotalCost'])){ ?>
                                          ...
                                          <span class="ab-3" style="padding-right: 10px;"><span class="selcurr subsellingPriceSum-<?=$subvalue['id']; ?> currencyConvert"><?php echo ' '.number_format(round($subvalue['sumSellingCost'],3,PHP_ROUND_HALF_UP),2); ?></span></span>
                                        <?php } else{ ?>
                                            ...
                                            <?php $k=1; foreach ($lineItems as $key => $lineItem) { 
                                              ...
                                              <form method="post" id="lineItemForm-<?= $lineItem['id']; ?>">
                                                <td style="width:13%" class="geeks">
                                                    <button type="button" cat_id="<?=$subvalue['id']; ?>" sid="<?= $insert_id; ?>" id="<?=$lineItem['id']; ?>" class="btn-33 delete_row"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
                          ...
                          <form method="post" action="" id="">
                            <div class="row">
                              <div class="form-group col-md-4">
                                  ...
                                  <input type="hidden" name="sub_cost_sheet_id" value="<?php echo $insert_id; ?>">
                    ...
                    <form method="post" action="" id="addCostCat" class='addCostCat' style='display: none; width: 40%; padding: 10px;'>
                      <div class="row">
                        <div class="form-group col-md-12" style="display: inline-flex;">
                          ...
                          <input type="hidden" name="cost_sheet_id" value="<?php echo $insert_id; ?>">
                ...
                <button class="btn btn-default btn-success" onclick="openCostCat('add-cost-cat',<?= $insert_id; ?>);"><i class="fa fa-plus" aria-hidden="true"></i> ADD</button>
              </div>
            </div>
            ...
            <div id="outer">
              <div class="inner">
                <form method="post" id="genrateTemplate">
                  <input type="hidden" name="costsheet_id" value="<?= $insert_id; ?>" />
                  <button type="button" class="btn msgBtn submitCostTemplate btn-success"><i class="fa fa-location-arrow" aria-hidden="true"></i>Submit </button>
                </form>
              </div>
            </div>
          </div>
...
function addMoreRow(id,cat_id)
{
  ...
  var lineItemData  = {'cat_id':cat_id,'sub_cat_id':id,'costsheet_id':<?= $insert_id; ?>};
  $.ajax({
            type:"POST",
            data:lineItemData,
          ...
          var newRow = $("<tr data-index='"+obj.lineItemID+"'><form method='POST' id='insertRowitem-"+id+"-"+increment+"'>");
            ...
            $("table.order-list-"+id+"").on("click", ".ibtnDel", function (event) {
              ...
              $.ajax({
                      ...
                      statusCode:{
                          200:function(data) {   
                            getTotalSellingPrice(id, <?= $this->uri->segment(5); ?>);
                          }
                      }
                    });
    ...
    $(document).on('change','.unit_costss, .unit_qty, .marginss', function() {
      ...
      $(document).ready(function() {
        ...
        $('#discount').on('keyup', function(){
          ...
          percents = percent;
          var deldata = {"percent":percent,'cost_sheet_id':<?php echo $insert_id; ?>};
          $.ajax({
            type:"POST",
            data:deldata,
        ...
        $('#example').DataTable();
          ...
          $(".delete_sub_cat").click(function(e){
              e.preventDefault();    
            var getvalue = $(this).attr("id");
            var deldata = {"id":getvalue,"table":'costsheet_subcategory','cost_sheet_id':<?php echo $insert_id; ?>};
          ...
          $(".delete_rowss").click(function(e){
            ...
            var deldata = {"id":getvalue,"table":'cost_sheet_category','cost_sheet_id':<?php echo $insert_id; ?>,'cat_id':catvalue};
      ...
      $(document).ready(function(){
        $('.submitCostTemplate').on('click', function(){
          ...
          var total_value = $('.sellingPriceSum').text();
          ...
          if(total_value == 0) {
              ...
              {
                genrateTemplate('genrateTemplate', 'generateCost');
              }
              else
              {
       ...
       if(currency == 'USD')
       {
          $("span.currencyConvert").each(function(index) { 
                $(this).text(formatNumber(Number.parseFloat(formatOldNumber($(this).html()) / convertVal).toFixed(2)));
          });
       }
