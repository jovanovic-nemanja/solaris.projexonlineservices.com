    						  ...
    						  <div class="row" style="margin-bottom: 20px">
    								...
    								<div class="col-3">
    									...
    									<form method="post" id="contact_person">
    											...
    											<select class="form-control select2" readonly="readonly"  id="contactPerson">
		    ...
		    <div class="row">
			    <div class="col-md-12 admin-accord">
				    <div id="accordionProject" class="accordion accordion-multi-colored" role="tablist">
					  <div class="card">
						<div class="card-header" role="tab" id="headingOne" style="background-color: #38ce3c;">
						  <h5 class="mb-0">
								...
								<div class="col-3">
									  <form method="post" id="venueForm">
											<label for="inputPassword4" class="">Venue</label>
											  <select class="form-control select2" readonly="readonly" id="venue">
                  ...
                  <div class="col-lg-12 admin-accord">
                    <div class="card-body">
                      ...
                      <?php } ?>
                              ...
                              ?>  
                                <div class="card">
                                      ...
                                      <h6 class="mb-0">
                                        ...
                                        <div class="aa-bb">
            							  	<?php if($value['sumSellingCost']){ ?>
                							    <span class="ab-2"><span class="totcurr totalcostsum-<?= $value['cat_id']; ?> currencyConvert"><?= number_format(round($value['sumTotalCost'],3,PHP_ROUND_HALF_UP),2); ?></span></span>&nbsp; | &nbsp;
                								<span class="ab-2 totavg-<?= $value['cat_id']; ?>"><?= round($value['sumTotalCost']/$value['sumSellingCost'],2); ?></span>&nbsp; | &nbsp;
                								<span class="ab-3"><span class="selcurr sellingPriceSum-<?= $value['cat_id']; ?> currencyConvert"><?= number_format(round($value['sumSellingCost'],3,PHP_ROUND_HALF_UP),2); ?></span></span>
            								 <?php } else{ ?>
            								    <span class="ab-2"><span class="totcurr totalcostsum-<?= $value['cat_id']; ?> currencyConvert"></span></span>
            							        <span class="ab-2 totavg-<?= $value['cat_id']; ?>"></span>
            								    <span class="ab-3"><span class="selcurr sellingPriceSum-<?= $value['cat_id']; ?> currencyConvert"></span></span>	
            								 <?php } ?>	
            								<button type="button"  id="<?php echo $value['cat_ids'] ?>" value="<?= $value['cat_id']; ?>" class="delete_rowss" style="color: #38ce3c;"><i class="fa fa-trash" aria-hidden="true"></i></button>
            							  </div>
                                      </h6>
                                    </div>
            										...
            										<h5 class="mb-0">
            											<a class="collapsed" role="button" data-toggle="collapse" href="#collapse-<?=$i?>-<?=$j?>" aria-expanded="false" aria-controls="collapse-1-1">
            												...
            												<div class="aaa-bbb" style="">
                                                                 <?php if(!empty($subvalue['quantity'])) { ?><span>Qty: <?= $subvalue['quantity']; ?></span><?php } ?>
                                                                 <?php if(!empty($subvalue['unit'])) { ?><span>Unit: <?= $subvalue['unit']; ?></span><?php }?>
                											</div>	
                											<div class="aa-bb" style="top: 0px;">
                												<?php if(!empty($subvalue['sumTotalCost'])){ ?>
                    												...
                    												<span class="ab-3"><span class="selcurr subsellingPriceSum-<?=$subvalue['id']; ?> currencyConvert"><?php echo ' '.number_format(round($subvalue['sumSellingCost'],3,PHP_ROUND_HALF_UP),2); ?></span></span>
                												<?php } else{ ?>
                												...
                												<button type="button" id="<?=$subvalue['id']; ?>" class="btn btn-dager delete_sub_cat"><i class="fa fa-trash" aria-hidden="true"></i></button>
                											</div>
            											</a>
            										</h5>
            									</div>
            													  ...
            													  <tr class="record" data-postion="<?php echo $lineItem['position']; ?>" data-index="<?php echo $lineItem['id']; ?>" >
            														<form method="post" id="lineItemForm-<?= $lineItem['id']; ?>">
            														 <td style="width:13%" class="geeks">
            															  <button type="button" cat_id="<?=$subvalue['id']; ?>" sid="<?= $this->uri->segment(4); ?>" id="<?=$lineItem['id']; ?>" class="btn-33 delete_row"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                  ...
                                  <?php $j++; } ?>
                                  		    ...
                                  		    <form method="post" action="" id="">
                                                <div class="row">
                                                    <div class="form-group col-md-4">
                                                        <input type="text" class="form-control" name="subcategory" id="subcategory" placeholder='Category'>
                                        ...
                                        <button class="btn btn-success add-rrr" value="<?=  $value['id'] ?>">ADD SUB-CATEGORY</button>
                                    </div>
                        ...
                        <div class='row'>
        			    <form method="post" action="" id="addCostCat" class='addCostCat' style='display: none; width: 40%; padding: 10px;'>
                            <div class="row">
        			        ...
        			        <table class="table">
        					    <tbody>
        						  <tr>
        							...
        							<td style="width:20%;"><p class="text-right mb-0"><strong>AED &nbsp; <span class="totalsellingPrice"><?= number_format(round($costSheetTotal[0]->sellingPriceSum,3,PHP_ROUND_HALF_UP),2); ?></span></strong></p></td>
        						  </tr>
        						  <tr>
        							<td style="width:80%;">
        								<div class="form-inline text-right mb-0">
        								   <span class="form-control" placeholder="Discount percent" id="email" style="width:64%; padding: 0.875rem 1.375rem;"> Discount</span>
        									<input type="text" class="form-control" value="<?php echo $costSheetData->discountPerent;  ?>" placeholder="" id="discount" style="width:30%; margin-left:20px;" >
        								 </div>	
        							</td>
        							<?php 
        							 ...
        							 ?>
        							<td style="width:20%;"><p class="text-right mb-0"><strong ><span class="discountPrice">AED <?= number_format(round($disPrice,3,PHP_ROUND_HALF_UP),2); ?></span></strong></p></td>
        						  </tr>
        						  <tr>
        							<td style="width:80%;"><p class="text-right mb-0"><strong>Total after discount</strong></p></td>
        							<td style="width:20%;"><p class="text-right mb-0"><strong><span class="totalarterDis">AED <?= number_format(round($totalPrice,3,PHP_ROUND_HALF_UP),2); ?></span></strong></p></td>
        						  </tr>
        						  <tr>
        							<td style="width:80%;"><p class="text-right mb-0"><strong>Vat @ 5% (that’s it) <span class="discountTotal"><?= number_format(round($totalPrice,3,PHP_ROUND_HALF_UP),2); ?></span></strong></p></td>
        							<td style="width:20%;"><p class="text-right mb-0"><strong><span class="vatPrice">AED <?= number_format(round($calculateVat,3,PHP_ROUND_HALF_UP),2); ?></span></strong></p></td>
        						  </tr>
        						  
        						  <tr>
        							<td style="width:80%;"><p class="text-right mb-0"><strong>Total including tax</strong></p></td>
        							<td style="width:20%;"><p class="text-right mb-0"><strong><span class="total">AED <?= number_format(round($totalCost,3,PHP_ROUND_HALF_UP),2); ?></span></strong></p></td>
        						  </tr>
        						</tbody>
        				    </table>
        				</div>
				    ...
				    <div id="outer">
					  ...
					  <div class="inner">
					  	<form method="post" id="genrateTemplate">
					  		<input type="hidden" name="costsheet_id" value="<?= $this->uri->segment(4); ?>">
					  		<button type="button" class="btn msgBtn btn-success submitCostSheet"><i class="fa fa-location-arrow" aria-hidden="true"></i>Submit</button>
					  	</form>
					  </div>
					</div>
                </div>
...
<div class="modal fade" id="add-cost-sub-cat" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Add  Sub Cost Category</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
            <form method="post" action="" id="addCostSubCat">
              <div class="row">
            ...
            success:function(response)
            {
                ...
                if(obj.err==0)
                {
					...
					var totalDiscount = percent;
      				...
      				$('.discountPrice').html('AED '+parseFloat(totalDiscount).toFixed(2));
      				$('.discountTotal, .totalarterDis').html('AED '+parseFloat(discountedprice).toFixed(2));
      				$('.vatPrice').html('AED '+parseFloat(vatTotal).toFixed(2));
      				$('.total, .esttotal').html('AED '+parseFloat(discountedprice+vatTotal).toFixed(2));
          ...
          $.ajax({
            ...
            url:"<?php echo base_url();?>index.php/admin/app/addCostSheetlineitem",
              ...
              {      
		        ...
		        //   $("span.totcurr").each(function(index){ 
	         //    		$(this).text(Number.parseFloat(+$(this).html() + +totalCost).toFixed(2));
    		 	  // });
		        });
		        ...
		        $('#total_cost-'+newId+',#margin-'+newId+',#qty-'+newId+',#unit_cost-'+newId+'').on('keyup change',function(){
		         ...
		         //  $("span.selcurr").each(function(index) { 
	          //   		$(this).text(Number.parseFloat(+$(this).html() + +totalsellingCost).toFixed(2));
    		 	   // });
	              });
              ...
              {      
                ...
                if(obj.err==0)
                {
					...
					var totalDiscount = percent;
      				...
      				$('.discountPrice').html('AED '+parseFloat(totalDiscount).toFixed(2));
      				$('.discountTotal, .totalarterDis').html('AED '+parseFloat(discountedprice).toFixed(2));
      				$('.vatPrice').html('AED '+(vatTotal).toFixed(2));
      				$('.total, .esttotal').html('AED '+parseFloat(discountedprice+vatTotal).toFixed(2));
      ...
      $(document).ready(function() {
              ...
              {      
                ...
                if(obj.err==0)
                {
                    ...
                    var totalDiscount = percent;
      				...
      				var vatTotal = (5/100)*(discountedprice);
      				$('.discountPrice').html(parseFloat(percent).toFixed(2));
      				$('.discountTotal, .totalarterDis').html(parseFloat(discountedprice).toFixed(2));
      				$('.vatPrice').html(parseFloat(vatTotal).toFixed(2));
      				$('.total, .esttotal').html(parseFloat(discountedprice+vatTotal).toFixed(2));
        ...
        $('#example').DataTable();
          ...
          if(confirm("Are you sure want to delete this record?"))
            ...
            var base_url = l.protocol + "//" + l.host + "/" + l.pathname.split('/')[1]+"/admin/app/";
      ...
      <script>
    	...
    	$(document).ready(function(){
    		$('.submitCostSheet').on('click', function(){
    			...
    			{
    				swal('Please fill all the field in customer Information');
    			}
